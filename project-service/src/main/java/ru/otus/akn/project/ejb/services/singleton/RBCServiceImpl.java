...
public class RBCServiceImpl implements RBCService {
    ...
    @Override
    public Map<String, String> getFreshNewsOrNull() throws IOException {
    ...
    @Override
    public Map<String, String> getFreshNews() throws IOException {
    ...
    private void fillUpNewsCache(Elements els) {
        for (Element el : els.subList(0, Math.min(MAX_NUMBER_OF_NEWS, els.size()))) {
            Elements href = el.getElementsByClass("main-feed__link");
            if (href.size() == 0) {
                continue;
